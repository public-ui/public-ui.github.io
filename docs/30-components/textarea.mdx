...
- Die Komponente rendert ein natives `<textarea>`-Element, das programmatisch mit einem `<label>`-Element verknüpft ist. Diese Verknüpfung stellt sicher, dass Screenreader das Eingabefeld korrekt ansagen.
...
- Pflichtfelder (`_required`) setzen das native `required`-Attribut sowie `aria-required="true"`, damit Screenreader Nutzende auf die Pflichtfeldkennzeichnung hinweisen.
- Im Fehlerfall wird `aria-invalid="true"` gesetzt, um den Fehlerzustand an assistive Technologien zu übermitteln.
- Deaktivierte Felder (`_disabled`) verwenden das native `disabled`-Attribut; schreibgeschützte Felder (`_readOnly`) setzen `readonly`.

### Beschriftung und Textalternativen

- Das Attribut `_label` ist verpflichtend und stellt sicher, dass jede Textarea eine sichtbare, programmatisch verknüpfte Beschriftung besitzt (WCAG 2.2 Erfolgskriterium 3.3.2).
...
Siehe [Tastatursteuerung](#tastatursteuerung) im Abschnitt Nutzung.
...
| 3.3.1 | Fehlererkennung — Fehlerhafte Eingaben werden mit `aria-invalid` und einer beschreibenden Fehlermeldung (`_msg`) kommuniziert | A |
| 3.3.2 | Beschriftungen oder Anweisungen — Jede Textarea erhält eine sichtbare Beschriftung über `_label` | A |
