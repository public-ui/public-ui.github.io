...
- Die Select-Komponente basiert auf dem nativen HTML-Element `<select>`.
- Der Browser weist dem nativen `<select>` automatisch die korrekten ARIA-Rollen zu und übernimmt zugleich die Tastaturbedienung, das Fokus-Handling und das Ansageverhalten für Screenreader.
...
<SelectPreview context="feature" visibleProperties={['_label', '_options']} />
...
<SelectPreview context="feature" visibleProperties={['_multiple', '_rows']} />

#### Formularattribute

Über `_disabled`, `_required` und `_name` wird die Select-Komponente in ein Formular eingebunden und ihr Eingabeverhalten gesteuert.

<SelectPreview context="feature" visibleProperties={['_disabled', '_required', '_name']} />

#### Hinweise und Fehlermeldungen

Mit `_hint` wird ein ergänzender Hinweis zur Eingabe ausgegeben, mit `_msg` eine Validierungs- oder Fehlermeldung.

<SelectPreview context="feature" visibleProperties={['_hint', '_msg']} />
