...
Die Komponente unterstützt Callbacks über die `_on`-Property sowie native DOM-Events:

| Event      | Auslöser                              | Value     |
| ---------- | ------------------------------------- | --------- |
| `click`    | Element wird angeklickt               | `boolean` |
| `toggle`   | Öffnungs- oder Schließzustand ändert sich | `boolean` |

Die `_on`-Property akzeptiert ein Objekt mit den Callbacks `onClick` und `onToggle`. Der Callback erhält das ursprüngliche `MouseEvent` sowie den neuen `_open`-Zustand (`true` = geöffnet) als Parameter.
