...
<InputDatePreview
  visibleProperties={['_label', '_type', '_disabled', '_required']}
  codeCollapsable
  codeCollapsed
/>
...
Die Icons in der Komponente sind per Tab-Taste nicht erreichbar. Die Auswahlhilfen der Komponente werden über die **Leertaste** oder **Enter** geöffnet. So erhalten Sie beispielsweise beim Typ **date** ein Kalendermodul, über das per Maus oder über die Tastatur das gewünschte Datum ausgewählt werden kann. Die manuelle Eingabe der Werte ist alternativ möglich.

### Bekannte Probleme

Es gibt <kol-link _href="/docs/known-issues#kol-input-date">bekannte Einschränkungen</kol-link> der Barrierefreiheit für das Input Date.
