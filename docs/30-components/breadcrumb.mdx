...
| `_icon`       | `string`  | (Optional) Der Name eines verfügbaren Icons (z.B. `kolicon-house` oder Font Awesome Symbole).               |
| `_hide-label` | `boolean` | (Optional) Wenn `true`, wird nur das Icon angezeigt und der Text verborgen. Requires `_icon` to be set.    |
