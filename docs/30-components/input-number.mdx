...
| `Pfeil-Tasten (oben/unten)` | Wert entsprechend der in `_step` festgelegten Schrittweite erhöhen/verringern. |
...
**Hinweis:** Sind `_min` und/oder `_max` gesetzt, kann der Wert über die Pfeiltasten nur innerhalb dieses Wertebereichs verändert werden. Bei manueller Eingabe können hingegen auch Werte außerhalb des definierten Bereichs eingegeben werden. Diese werden erst durch die native Browservalidierung berücksichtigt.
