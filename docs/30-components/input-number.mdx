...
Verwenden Sie `_min` und `_max`, um den zulässigen Wertebereich einzuschränken. Mit `_step` legen Sie fest, in welchen Schritten der Wert angepasst werden kann. Beachten Sie aber: `_min`, `_max` und `_step` verhindern keine manuelle Eingabe außerhalb des zulässigen Wertebereichs. Die Validierung manuell eingegebener Werte muss auf Anwendungsebene erfolgen.
