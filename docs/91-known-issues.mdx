...
- **Browserabhängige Darstellung**: Das Erscheinungsbild des `<select>`-Elements variiert je nach Browser und Betriebssystem, was zu uneinheitlichen Nutzererfahrungen führt.
  - [Chromium basierte Browser](https://developer.chrome.com/blog/a-customizable-select) bieten mittlweile die Möglichkeit das Styling von `<select>`-Elementen über die CSS Property `appearance:base-select` anzupassen. Dies ändert teilweise auch die [Tastatursteuerung](https://open-ui.org/components/customizable-select.explainer/#keyboard-behavior). Es kann sein, dass heir in den nächsten Monaten noch einige Anpassungen folgen werden, bis sich der neue Standard und die damit einhergehende UX gefestigt und durchgesetzt haben.
