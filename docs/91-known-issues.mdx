...
- Auto Complete / Auto Fill: NVDA liest die Auswahlmöglichkeiten nicht immer vor

[NVDA GitHub Issue dazu](https://github.com/nvaccess/nvda/issues/17360)
...
- **Browserabhängige Darstellung**: Das Erscheinungsbild des `<select>`-Elements variiert je nach Browser und Betriebssystem, was zu uneinheitlichen Nutzererfahrungen führt.
  - [Chromium basierte Browser](https://developer.chrome.com/blog/a-customizable-select) bieten mittlerweile die Möglichkeit das Styling von `<select>`-Elementen über die CSS Property `appearance:base-select` anzupassen. Dies ändert teilweise auch die [Tastatursteuerung](https://open-ui.org/components/customizable-select.explainer/#keyboard-behavior). Es kann sein, dass bei den Browsern in den nächsten Monaten noch einige Anpassungen folgen werden, bis sich der neue Standard und die damit einhergehende UX gefestigt und durchgesetzt haben.

- **Multiple mit Rows=1**: Auch hier unterscheiden sich die Browser sehr. Chromium Browser zeigen dies als Multiselect mit Dropdown an. FF rendert einen einzeiligen Multiselect Container - siehe https://github.com/public-ui/kolibri/issues/10453
...
KoliBri verwendet das native <code>&lt;dialog&gt;</code>-Element. Deswegen muss folgendes beachtet werden:
- cancel Event wird bei einem Abbruch durch den Nutzer (z. B. ESC-Taste) gefeuert.
- close Event feuert immer beim endgültigen Schließen (egal ob durch API, Formular oder nach einem Abbruch).
- Wenn man im cancel-Event event.preventDefault() aufruft, bleibt der Dialog beim ersten Druck auf die ESC-Taste offen. Drückt der Nutzer jedoch ein zweites Mal ESC, ohne in der Zwischenzeit mit dem Dokument zu interagieren (z. B. durch einen Klick), wird der Dialog trotzdem zwangsgeschlossen -> User soll  nicht eingesperrt werden können. (Dieses Verhalten ist noch ziemlich neu und nicht in allen Browsern konsistent umgesetzt.)
