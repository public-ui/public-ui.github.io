...
<html lang="de" dir="ltr">
	...
	<body>
		<main class="itzbund" data-theme="itzbund">
			<kol-link _href="https://public-ui.github.io" _label="" _target="github">
				<kol-kolibri slot="expert" _labeled="false"></kol-kolibri>
			</kol-link>
			...
			<p><kol-icon _aria-label="" _icons="fa-solid fa-arrow-right"></kol-icon> Link zur ausführliche <kol-link _href="https://public-ui.github.io" _label="Dokumentation" _target="github"></kol-link></p>
...
GROUPED_PROPS.forEach((group) => {
	group.forEach((name) => {
		...
		PROP_DESCS.forEach((_desc, idx) => {
			MARKDOWN += `| ${name} | ${PROP_TYPES.map((type) => type.replace(/\|/g, '&#124;')).join(', ')} | ${PROP_DESCS[
				idx
			].replace(/\n/g, '')} |
`;
		});
...
SHEET_CHEAT += `
			...
			<p style="text-align: center; margin:1rem 2rem 0 2rem;><strong><kol-abbr _title="Komponenten-Bibliothek für die Barrierefreiheit" _tooltip-align="right">KoliBri</kol-abbr></strong> ist unter der Lizenz <kol-link _href="https://github.com/public-ui/kolibri/blob/main/LICENSE" _label="" _target="eu"><strong slot="expert">EUPL v1.2</strong></kol-link> lizenziert.</p>
		</main>
	</body>
</html>`;

const FOLDER = 'static/cheat-sheet/';
fs.writeFileSync(`${FOLDER}index.html`, SHEET_CHEAT);
fs.writeFileSync(`${FOLDER}CHEAT_SHEET.md`, MARKDOWN);
