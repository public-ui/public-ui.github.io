...
The component supports callbacks via the `_on` property as well as native DOM events:

| Event    | Trigger                              | Value     |
| -------- | ------------------------------------ | --------- |
| `click`  | Element is clicked                   | `boolean` |
| `toggle` | Open or closed state changes         | `boolean` |

The `_on` property accepts an object with the callbacks `onClick` and `onToggle`. The callback receives the original `MouseEvent` and the new `_open` state (`true` = open) as parameters.
