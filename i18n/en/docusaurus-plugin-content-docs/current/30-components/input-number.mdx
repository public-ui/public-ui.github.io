---
title: InputNumber
description: Description, specification and examples for the InputNumber component.
---

import Readme from '@site/readmes/input-number/readme.md';
import InputNumberPreview from '@site/src/components/previews/components/InputNumber';
import { ExampleLink } from '@site/src/components/ExampleLink';
import { PropertyMsgUsage } from '@site/src/components/PropertyMsgUsage';
import BetaDocsBanner from '@site/src/components/BetaDocsBanner';


# InputNumber


<BetaDocsBanner />

The input type **Number** creates an input field for numbers.

## Construction

<InputNumberPreview context="feature" visibleProperties={['_label', '_disabled', '_readOnly', '_required']} />

### Events

For the handling of events or callbacks, see <kol-link _label="Events" _href="../concepts/events" />.

| Event | trigger | Value |
|----------|----------------------------------|---------------------------------|
| `click` | Input field is clicked | - |
| `focus` | Input field is focused | - |
| `blur` | Input field loses focus | - |
| `input` | Value is changed by entering | Current value of the input field |
| `change` | Entry has been completed | Current value of the input field |

## Usage

### Best practices

- Make sure to set `id` and `name` correctly so that the data is sent when you submit the form.

### Keyboard controls

| button | Function |
| ----------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Tab` | Focuses the input field.                                                                                                                          |
| `Arrow keys (up/down)` | Increases or decreases the value within the **`_min`** and **`_max`** attributes according to the step size specified in the **`_step`** attribute. |

### SmartButton

A button with an arbitrary action can be placed within the field (`_hide-label` only):

<InputNumberPreview
	context="feature"
	initialProps={{ _label: 'With SmartButton', _hideLabel: true, _placeholder: 'Placeholder', _smartButton: { _label: 'Action' } }}
	visibleProperties={['_smartButton']}
/>

### Suggestions

Use `_suggestions` to provide a list of suggested values for autocomplete:

<InputNumberPreview
	context="feature"
	visibleProperties={['_label', '_suggestions']}
	initialProps={{
    _label: 'Quantity',
    _suggestions: [1, 5, 10, 25, 50, 100],
  }}
/>

## Links and references

- <kol-link _href="https://medium.com/@gavyn/til-autofocus-inputs-are-an-accessibility-problem-32ced60c3109" _target="_blank"></kol-link>

## API

<Readme />

## Usage _msg

<PropertyMsgUsage />

## Examples

<ExampleLink component="input-number" />
