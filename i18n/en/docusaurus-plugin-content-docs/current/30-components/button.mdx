...
The button supports the following ARIA attributes:

| Property | Description |
| --- | --- |
| `_ariaControls` | Defines which elements are controlled by this button (`aria-controls`). |
| `_ariaDescription` | Sets the `aria-description` attribute for an extended description read by screen readers. |
| `_ariaExpanded` | Indicates whether the button controls an expanded element (`aria-expanded`). |
| `_ariaSelected` | Indicates whether the interactive element is selected, e.g. for `role="tab"` (`aria-selected`). |
