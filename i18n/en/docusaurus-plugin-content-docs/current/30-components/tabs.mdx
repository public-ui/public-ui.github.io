...
<TabsPreview context="feature" visibleProperties={['_label', '_tabs', '_align', '_hasCreateButton']} />
...
The `_behavior` attribute controls how users switch between tabs:

- `"select-automatic"` (default): Arrow keys switch the active tab directly (W3C standard).
- `"select-manual"`: Arrow keys move focus only; `Enter` / `Space` activates the focused tab.
...
| `Enter` / `Space` (with `select-manual`) | Activates the focused tab. |

## FAQ

**How is the active tab determined?**  
By default, the first tab (index 0) is active on initial load. You can set the active tab manually using the `_selected` attribute or control it via the `select` event.

**Can a tab be disabled?**  
Yes, by setting the `_disabled` attribute to `true` in the tab definition.

**What behaviors (`_behavior`) are available?**  
`"select-automatic"` (default): Arrow keys switch focus and selection simultaneously.  
`"select-manual"`: Arrow keys move focus only; selection is made with `Enter` / `Space`.
