...
The **TableStateless** component is responsible for purely displaying the KoliBri table.

## Construction

For a table component that can automatically sort and paginate with the data provided, see <kol-link _href="table-stateful" _label="KolTableStateful" />.
...
The settings menu can be activated by setting `_hasSettingsMenu={true}`. If users change the settings, they are automatically transferred to the table view and a <nobr>`settingsChange`</nobr> DOM event is triggered (see Events).
...
| `changeHeaderCells` | The header cells were updated via `_on.onChangeHeaderCells`. | Updated header data as `TableHeaderCells`. |
